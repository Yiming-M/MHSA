...
class SuMoCoDDP(nn.Module):
    def __init__(
        ...
        This module will return encoded features and their corresponding labels. Loss will be calculated by SupCon.
    ...
    def __init_params_k__(self) -> None:
        ...
        for param_q, param_k in zip(self.mlp_q.parameters(), self.mlp_k.parameters()):
            param_k.data.copy_(param_q.data)  # initialise
            param_k.requires_grad = False  # not updated by gradient
    ...
    def __dequeue_and_enqueue__(self, feats: Tensor, labels: Tensor) -> None:
        # gather feats and labels before updating queue
